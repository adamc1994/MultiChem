% Lactose1 isomerisation test problem.
...
for i=1:1:(size(conditions(:,1),1))
            ...
            [Cfinal, Yield, TimeData, ConcData] = Lactose_Constants(ConcA, T(i) * ones(1,4), tR(i));
            ...
            STY_ = STY_calc(product/100, tR(i), ConcData(1,1), 342.30);
            ...
            f = [-log(product_yield) log(side1_yield)]; % maximise product yield, minimise by-product yield
...
% Determine number of Pareto optimal solutions after each iteration
            ...
            Side_yield = exp(data.y(:,2));

            data.z = [Product_yield -Side_yield];
